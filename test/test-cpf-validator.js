...
describe('Cpf validator', ()=>{
    it('1°: Deve retornar true se o validador retornar true', ()=>{
        const sut = makeSut();
        const isCpfValid = sut.isCpfValid('12345678909');
        assert.isTrue(isCpfValid);
    })

    it('2°: Deve retornar false se o validador retornar false', () => {
        const sut = makeSut();
        const isCpfValid = sut.isCpfValid('00000000000');
        assert.isFalse(isCpfValid);
    })

    it('3°: Deve fazer um throw se não for fornecido um cpf', () => {
    const sut = makeSut()
    const invalid = null
    ...
    it('4°: Deve fazer um throw se o cpf fornecido for não numérico', () => {
    const sut = makeSut()
